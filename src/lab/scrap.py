...
def writefile(fname,s,s1,heading):
	...
	content=f.read()
	#print content
	filedata=content.replace('index.html','Aim.html?domain= Chemical Engineering&lab='+heading)
	f.seek(0)
	f.write(filedata)	
...
print heading
breadcrumb='<a href="http://vlabs.ac.in/chemical-engineering-labs.html" class="sidebar-a" >Chemical Engineering </a><br/>'
while sectionNumber<=len(sectionno):
	...
	if tag1=='Prerequisite S/W':
		tag1='Prerequisites'
	st+='<a href="'+tag1+'.html?domain=Chemical Engineering"'+' class="sidebar-a" > <h3 class="text-h3-darkblue" style="margin-top: 2px;">'+tag1+'</h3></a>'	
	sectionNumber=sectionNumber+1
st+='<a href="Feedback.html?domain=Chemical Engineering" class="sidebar-a" > <h3 class="text-h3-darkblue" style="margin-top: 2px;">Feedback</h3></a>'
sectionNumber=1
while sectionNumber<=len(sectionno):
	tag=""
	tag=""
